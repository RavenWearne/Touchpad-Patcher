...
concise_native_error() {
	local reason owner
	if grep -Eq "owned by .+ GRUB.*generated by os-prober" "$log_file"; then
		owner=$(sed -n "s/.*owned by \([^ ]*\) GRUB.*/\1/p" "$log_file" | head -n1)
		reason="This Linux installation must be patched through ${owner:-the bootloader-owning system} GRUB."
	else
...
if (( native_status == 20 )); then
	python3 "$user_summary" multi "$inventory_file"
	apply_native_and_report || apply_status=$?
